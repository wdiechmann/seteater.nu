...
Start by reading through [this 6 part guide on setting up the app and environment for deployment of a Rails app on Debian](http://vladigleba.com/blog/2014/03/05/deploying-rails-apps-part-1-securing-the-server/)
		...
		spring rails server
		mate .env config/production.rb config/seteater.nginx.conf config/secrets.yml config/unicorn_init.sh config/unicorn.rb 
		rake secret
		...
		sudo update-rc.d unicorn_seteater defaults
...
* sudo service nginx restart
* /etc/init.d/unicorn_seteater restart
